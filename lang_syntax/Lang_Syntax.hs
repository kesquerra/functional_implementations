...
cmd (Pen dir) (mode, point) = ((dir, point), Nothing)
cmd (Move x y) (mode, point) = ((mode, (expr x, expr y)), line)
    where line = if mode == Down then Just (point, (expr x, expr y)) else Nothing



-- | Get the state out of block
--
getState :: (State, [Line]) -> State
getState (state, _) = state


-- | Get the Line list out of block
--
getLines :: (State, [Line]) -> [Line]
getLines (_, line) = line
...
block [] state = (state, [])
block (c:cmds) state = case cmd c state of
    (newstate, Nothing) -> block cmds newstate
    (newstate, Just line) -> (getState (block cmds newstate), line: getLines(block cmds newstate))
...
optimize :: Prog -> Prog
optimize [] = []
optimize (c:cmds) = case c of
    Pen dir -> Pen dir: optimize cmds
    Move x y -> Move (Lit (expr x)) (Lit (expr y)): optimize cmds
