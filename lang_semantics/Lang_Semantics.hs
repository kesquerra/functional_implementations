...
checkExpr vars (Ref x) = x `elem` vars
checkExpr vars (Add x y) = checkExpr vars x && checkExpr vars y
checkExpr vars (Mul x y) = checkExpr vars x && checkExpr vars y
...
checkCmd env vars (Move x y) = checkExpr vars x && checkExpr vars y
checkCmd env vars (Call m a) = all (checkExpr vars) a && macDef
  where
    macDef = case get m env of
      Just v -> v == length a
      Nothing -> False
checkCmd env vars (For v x y b) = checkExpr vars x && checkExpr vars y && checkBlock env allvars b
  where
    allvars = vars ++ [v]
...
checkBlock env vars = all (checkCmd env vars)
...
checkDef env (Define m p b) = checkBlock env p b
...
expr :: Env -> Expr -> Int
expr _ (Lit i) = i
expr env (Add x y) = (expr env x) + (expr env y)
expr env (Mul x y) = (expr env x) * (expr env y)
expr env (Ref x) = case get x env of
                      Just v -> v
...
cmd defs env state@(pen,pos) c = case c of
    ...
    Move xExp yExp -> case pen of
                        Down -> ((pen, p), [(pos, p)])
                          where p = (expr env xExp, expr env yExp)
                        Up -> ((pen, p), [])
                          where p = (expr env xExp, expr env yExp)


    Call macro args -> block defs newenv state b
      where 
        x@(pars, b) = getOrFail macro defs
